...
def main():
    ...
    parser.add_argument(
        "-i", "--input-dir", type=str, default=None, help=("Inpute directory with delta files")
    )
    parser.add_argument(
        "-o",
        "--output-dir",
    ...
    parser.add_argument("-n", "--name", type=str, default=None, help=("Name of run"))
    parser.add_argument(
        "--rp-min", type=float, default=0, required=False, help=("Minimum line-of-sight separation")
    )
    parser.add_argument(
        "--rp-max",
        type=float,
    ...
    parser.add_argument(
        "--rt-max", type=float, default=200, required=False, help=("Maximum transverse separation")
    )
    parser.add_argument(
        "--num-bins-rp",
        type=int,
    ...
    parser.add_argument(
        "--z-cut-min", type=float, default=0, required=False, help=("Minimum redshift")
    )
    parser.add_argument(
        "--z-cut-max", type=float, default=10, required=False, help=("Maximum redshift")
    )
    parser.add_argument(
        "--absorption-line",
        type=str,
    ...
    parser.add_argument(
        "--Omega-m", type=float, default=0.315, required=False, help=("Matter density parameter")
    )
    parser.add_argument(
        "--Omega-r", type=float, default=0, required=False, help=("Radiation density parameter")
    )
    parser.add_argument(
        "--no-project", action="store_true", required=False, help=("Turn off projection matrix")
    )
    parser.add_argument(
        "--use-new-projection",
        action="store_true",
    ...
    config["cosmology"] = {"Omega_m": str(args.Omega_m), "Omega_r": str(args.Omega_r)}

    config["compute"] = {"compute-correlation": "True"}

    config["output"] = {"name": args.name, "output-dir": args.output_dir}
