...
class Cosmology:
    ...
    def __init__(self, config):
        ...
        if config.getboolean("use-picca-cosmo"):
            ...
            self._inv_efunc_args = (
                config.getfloat("omega_m"),
                config.getfloat("omega_r"),
                self._Omega_k,
                config.getfloat("w0"),
            )
    ...
    def _comoving_distance(self, z):
        ...
        if isinstance(z, (list, tuple, np.ndarray)):
            return self._hubble_distance * np.array(
                [self._comoving_distance_scalar(z_scalar) for z_scalar in z]
            )
        else:
...
class PiccaCosmo:
    def __init__(self, config):
        ...
        for index in range(1, num_bins):
            r_comov[index] = (
                SPEED_LIGHT * (1.0 / hubble[index - 1] + 1.0 / hubble[index]) / 2.0 * delta_z
                + r_comov[index - 1]
            )
        ...
        # D_V
        dist_v = np.power(z * self.get_dist_m(z) ** 2 * self.get_dist_hubble(z), 1.0 / 3.0)
